...
class MyBadgesTab extends StatelessWidget {
  ...
  const MyBadgesTab({Key? key, required this.challengeBadges, required this.engagementBadges})
      : super(key: key);
    ...
    Widget getChallengeBadges() {
      if (challengeBadges.isNotEmpty) {
        return ListView(
          ...
          shrinkWrap: true,
          children: [
            Text('Udfordringsmærker', style: TextStyle(color: Colors.white, fontSize: 20)),
            GridView.count(
                physics: NeverScrollableScrollPhysics(), // to disable GridView's scrolling
                shrinkWrap: true,
                mainAxisSpacing: 10,
